...
fs.writeFile('firewall-rules.sh', iptablesCommands, (err) => {
  if (err) {
    console.error('Error writing shell script file:', err);
    return;
  }

  exec('chmod +x firewall-rules.sh', (error, stdout, stderr) => {
    if (error) {
      console.error('Error setting execute permissions:', error.message);
      return;
    }
    if (stderr) {
      console.error('stderr from chmod command:', stderr);
      return;
    }
    console.log('Shell script created and permissions set.');

    exec('./firewall-rules.sh', (error, stdout, stderr) => {
      if (error) {
        console.error('Error executing shell script:', error.message);
        return;
      }
      if (stderr) {
        console.error('stderr from shell script execution:', stderr);
        return;
      }
